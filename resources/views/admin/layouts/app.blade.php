...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav ml-auto">
       @if(Auth::user()->type_agence == 'LOCATION DES VEHICULES')
        <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#">
          <i class="fa fa-car"></i>

          <span class="badge badge-danger navbar-badge">{{ count(Reservation(Auth::user()->id)) }}</span>
        </a>
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
           <span class="dropdown-item dropdown-header">{{ count(Reservation(Auth::user()->id)) }} Demande de réservation</span>
            <div class="dropdown-divider"></div>
            @foreach(Reservation(Auth::user()->id) as $keyMessage => $reservation)
          <a href="{{ url('/admin/reservation/'.$reservation->id) }}" class="dropdown-item">
            <!-- Message Start -->
             
            <div class="media">
              <div class="media-body">
                <h3 class="dropdown-item-title">
                   <span style="color: red">{{Client($reservation->id_client)->first_name}}-{{Client($reservation->id_client)->last_name}}<br></span>
                  {{$reservation->type_resevation}}
                  
                <p class="text-sm text-muted"><i class="far fa-clock mr-1"></i> {{date_format($reservation->created_at,'Y-M-d | H:i')}}</p>
              </div>
            </div>
          ...
          <a href="{{url('/admin/reservation') }}" class="dropdown-item dropdown-footer">Afficher tout les réservation</a>
        </div>
      </li>
      @elseif(Auth::user()->type_agence == 'LOCATION DES APPARTEMENTS')
    <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#">
      ...
      <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#">
          <i class="fa fa-bank"></i>
          <span class="badge badge-danger navbar-badge">{{ count(Demande()) }}</span>
        </a>
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
           <span class="dropdown-item dropdown-header">{{ count(Demande()) }} Demande</span>
            <div class="dropdown-divider"></div>
            @foreach(Demande() as $keyMessage => $valueMessage)
          <a href="{{ url('/admin/demande/'.$valueMessage->id) }}" class="dropdown-item">
            <!-- Message Start -->
            ...
            <div class="media">
              <div class="media-body">
                ...
                <p class="text-sm" style="color: red">{{ $valueMessage->type_agence}}</p>
                <p class="text-sm text-muted"><i class="far fa-clock mr-1"></i> {{date_format($valueMessage->created_at,'Y-M-d | H:i')}}</p>
              </div>
            </div>
          ...
          <a href="{{url('/admin/demande') }}" class="dropdown-item dropdown-footer">Afficher tout les Demandes</a>
        </div>
      </li>
      @endif
